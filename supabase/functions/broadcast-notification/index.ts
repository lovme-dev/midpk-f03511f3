...
async function generateVapidJWT(endpoint: string, vapidPrivateKey: string, vapidPublicKey: string): Promise<string> {
  ...
  const subject = 'mailto:support@middasbuy.com';
...
Deno.serve(async (req) => {
  ...
  try {
    const { title, message, type, icon_url, action_url, admin_id } = await req.json();
    ...
    console.log(`[Broadcast] Sending notification to all users: ${title}`);
    ...
    if (notifError) {
      ...
      return new Response(
        JSON.stringify({ error: 'Failed to create notification: ' + notifError.message }),
        { status: 500, headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
      );
    }
    ...
    // 2. Get ALL registered users from profiles table (not just push subscribers)
    ...
    // 3. Create user_notifications for ALL registered users so they see it in bell/inbox
    if (allProfiles && allProfiles.length > 0) {
      ...
      if (userNotifError) {
        ...
        console.log(`[Broadcast] Created user_notifications for ${allProfiles.length} users`);
      }
    }

    // 4. Get push subscriptions for sending device notifications
    ...
    if (!subscriptions || subscriptions.length === 0) {
      return new Response(
        JSON.stringify({ 
          success: true, 
          notification_id: notification.id,
          message: 'Notification saved to inbox, no push subscriptions found',
          sent: 0,
          total: 0,
          users_in_inbox: allProfiles?.length || 0
        }),
    ...
    for (const subscription of subscriptions) {
      try {
        ...
        if (response.ok || response.status === 201) {
          successCount++;
          console.log(`[Broadcast] Successfully sent to ${subscription.user_id}`);
          // Update user_notification as delivered
          await supabase
            .from('user_notifications')
          ...
          // 403 = VAPID key mismatch (old/bad subscription), 410/404 = expired
          expiredSubscriptions.push(subscription.id);
          console.log(`[Broadcast] Subscription invalid for ${subscription.user_id}: ${response.status} (will be removed)`);
        } else {
    ...
    if (expiredSubscriptions.length > 0) {
      console.log(`[Broadcast] Removing ${expiredSubscriptions.length} expired subscriptions`);
      await supabase
        .from('push_subscriptions')
        .delete()
    ...
    console.log(`[Broadcast] Successfully sent push to ${successCount}/${subscriptions.length} subscriptions`);
